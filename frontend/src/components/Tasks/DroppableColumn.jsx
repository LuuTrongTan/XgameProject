 ...
 * @param {Array} props.tasks - Danh sách task trong cột
...
const DroppableColumn = ({
  ...
  React.useEffect(() => {
    if (isOver) {
      console.log(`Column ${status} is now OVER`);
    }
  }, [isOver, status]);

  // Chuẩn bị dữ liệu cho SortableContext
  const sortableItems = tasks?.map(task => task._id) || [];
  ...
  return (
    <Card
      ...
      sx={{
        ...
        backgroundColor: isOver ? `rgba(${color.replace('#', '').match(/../g).map(h => parseInt(h, 16)).join(', ')}, 0.05)` : '#f8f9fc',
        borderRadius: '20px',
        boxShadow: isOver ? '0 16px 32px rgba(0,0,0,0.15)' : '0 8px 24px rgba(0,0,0,0.08)',
        border: isOver ? `2px dashed ${color}` : `1px solid ${isOver ? `${color}` : borderColor}`,
      ...
      onClick={() => {
        console.log(`Column ${status} clicked - id: ${status}, has correct attributes: ${
          document.getElementById(status)?.getAttribute('data-status') === status
        }`);
      }}
    >
      <CardContent 
